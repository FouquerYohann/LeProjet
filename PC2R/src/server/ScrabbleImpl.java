...
public class ScrabbleImpl implements ScrabbleService {
	...
	if (more.size() > 7) {
	    System.out.println("not is valid placement more size > 7");
	    return false;
	}
	...
	for (Lettre lettre : more) {
		...
		System.out
		        .println("not is valid placement not contained in tirage");
		return false;
	    }
	}

	ScrabbleService tmp=new ScrabbleImpl();
	tmp.init(ScrabbleParser.parseGrille(placement), copieLettreTire.toArray(new Lettre[copieLettreTire.size()]));
	
	IDico dick = new Dico();
	for (Lettre[] lettre : tmp.getListeMot()) {
	   if(!dick.isMotValide(lettre)){
		System.out.println(Lettre.toString(lettre)+ "n'est pas un mot");
		return false;
	    }
        }
